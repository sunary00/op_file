...
int find_free_db()
{
	...
	for (int i = 1; i < 100; i++)
	{
		if (ctx.superblock->data_bitmap[i] == '0')
		{
			ctx.superblock->data_bitmap[i] = '1';
			printf("free i=%d\n", i);
			return i;
		}
	}
...
{
	printf("MKDIR\n");
	printf("the path is %s, the mode is %d",path,mode);
...
{ // ls children
	printf("READDIR\n");
	printf("the input is path %s ,buffer");
...
{ // vi  cat
	...
	else
	{
		...
		{
			strncat(str, &(ctx.superblock->datablocks[BLOCKSIZE * (file->datablocks[i])]), 1024);
			printf("--> %s", str);
		}
		strncat(str, &(ctx.superblock->datablocks[BLOCKSIZE * (file->datablocks[i])]), (file->size) % 1024);
		printf("--> %s", str);
		// strncpy(str, &spblock.datablocks[BLOCKSIZE*(file -> datablocks[0])], file->size);
		strcpy(buf, str);
	}
	printf("READ end\n");
	return file->size;
...
{ // quanxianjiancha
	...
	printf("myaccess input is path: %s, mask: %d not yet\n",path, mask);
	return 0;
}
...
{ // wen jian jieduan  > /xx
	printf("mytruncate input is path: %s, size: %d not yet\n",path, size);
	char *pathname = malloc(sizeof(path) + 1);
	strcpy(pathname, path);
	filetype *file = filetype_from_path(pathname);
	if (file == NULL)
	...
	if (file->size > size){
		...
		for (int i = (BLOCKSIZE * ((file->datablocks)[0])+size); i < (BLOCKSIZE * ((file->datablocks)[0]) + (file->size)); i++)
		{
			ctx.superblock->datablocks[i] = 0;
		}
		file->size = size;
	}
	else{
...
{ // echo 'aaa'>/xx
	...
	if (file->size == 0)
	{
		...
		strcpy(&(ctx.superblock->datablocks[BLOCKSIZE * ((file->datablocks)[0])]), buf);
		printf("file->datablocks0: %d\n",file->datablocks[0]);
	...
	else
	{
		...
		if (len1 >= strlen(buf))
		{
			strcat(&(ctx.superblock->datablocks[BLOCKSIZE * ((file->datablocks)[currblk])]), buf);
			file->size += strlen(buf);
			printf("---> %s\n", &(ctx.superblock->datablocks[BLOCKSIZE * ((file->datablocks)[currblk])]));
		}
		else
		{
			char *cpystr = malloc(1024 * sizeof(char));
			strncpy(cpystr, buf, len1 - 1);
			strcat(&(ctx.superblock->datablocks[BLOCKSIZE * ((file->datablocks)[currblk])]), cpystr);
			strcpy(cpystr, buf);
			strcpy(&(ctx.superblock->datablocks[BLOCKSIZE * ((file->datablocks)[currblk + 1])]), (cpystr + len1 - 1));
			file->size += strlen(buf);
			printf("---> %s\n", &(ctx.superblock->datablocks[BLOCKSIZE * ((file->datablocks)[currblk])]));
			(file->blocks)++;
		}
